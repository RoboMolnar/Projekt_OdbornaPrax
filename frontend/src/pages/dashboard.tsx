...
export default function DashboardGarant() {
  ...
  // ⬇⬇⬇ Po úspechu zavrieme detail a refreshneme zoznam ⬇⬇⬇
      ...
      <div className="grid grid-cols-1 gap-4">
        <Card>
          <CardHeader className="flex flex-row items-center justify-between gap-3">
            <div>
              <CardTitle>Prehľad praxí</CardTitle>
            </div>
          </CardHeader>
                  ...
                  {!loading &&
                    rows.map((r) => (
                      <TableRow key={r.id} className="hover:bg-slate-50">
                        <TableCell><span className="text-slate-900">{r.student || '—'}</span></TableCell>
                        ...
                        <TableCell className="text-right space-x-2">
                          {r.status === 'Vytvorená' && (
                            <>
                              <Button size="sm" onClick={() => approve(r.id)}>Schváliť</Button>
                              <Button size="sm" variant="secondary" onClick={() => reject(r.id)}>Zamietnuť</Button>
                            </>
                          )}
                          {r.status === 'Schválená' && (
                            <>
                              <Button size="sm" onClick={() => grade(r.id, true)}>Ohodnotiť: Prešiel</Button>
                              <Button size="sm" variant="secondary" onClick={() => grade(r.id, false)}>Neprešiel</Button>
                            </>
                          )}
                          <Button size="sm" variant="ghost" onClick={() => openDetail(r.id)}>Detail</Button>
                          <Button size="sm" variant="destructive" onClick={() => removeInternship(r.id)}>Vymazať</Button>
                        </TableCell>
                      </TableRow>
                    ))}
                </TableBody>
          ...
          <div className="w-full max-w-lg rounded-xl bg-white shadow-lg border border-slate-200 p-6">
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <p className="text-sm text-slate-500">Podrobné informácie o odbornej praxi.</p>
              </div>
              <button
                type="button"
                onClick={closeDetail}
            ...
            <div className="mt-4 text-sm">
              {detailLoading && <p className="text-slate-500">Načítavam…</p>}
                ...
                <div className="space-y-4">
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      ...
                      <p className="text-slate-900">
                        ...
                        {selected.student_email ? <span className="block text-slate-500">{selected.student_email}</span> : null}
                      </p>
                    </div>
                    <div>
                      <p className="font-semibold text-slate-700">Odbor</p>
                      <p className="text-slate-900">{selected.program ?? '—'}</p>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-slate-900">{selected.year} / {selected.semester}</p>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      ...
                      <p className="text-slate-900">{selected.worked_hours ?? '—'}</p>
                    </div>
                    <div>
                      <p className="font-semibold text-slate-700">Stav</p>
                      <Badge className={STATUS_CLASSES[selected.status] || 'border-slate-300'}>
                        {selected.status}
                      </Badge>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-2 items-end pt-2">
                    <div className="md:col-span-2">
                      <label className="block text-slate-700 font-semibold mb-1">Zmeniť stav</label>
                      <select
                      ...
                      <p className="mt-1 text-xs text-slate-500">Uloženie stavu teraz automaticky zatvorí detail.</p>
                    </div>
                  ...
                  <div className="flex flex-wrap gap-2 pt-4">
                    {selected.status === 'Vytvorená' && (
                      <>
                        <Button onClick={() => approve(selected.id)}>Schváliť</Button>
                        <Button variant="secondary" onClick={() => reject(selected.id)}>Zamietnuť</Button>
                      </>
                    )}
                    {selected.status === 'Schválená' && (
                      <>
                        <Button onClick={() => grade(selected.id, true)}>Ohodnotiť: Prešiel</Button>
                        <Button variant="secondary" onClick={() => grade(selected.id, false)}>Neprešiel</Button>
                      </>
                    )}
                    <Button variant="destructive" onClick={() => removeInternship(selected.id)}>Vymazať</Button>
                  </div>
                </div>
              )}
            </div>
